...
function addClickHandlers() {
    ...
    clearFriendsBtn.addEventListener("click", function (e) {
        chrome.runtime.sendMessage({ action: "clearFriends" }, function (response) {
    ...
    activateExtBtn.addEventListener("click", function (e) {
        if(activated) chrome.runtime.sendMessage({ action: "deactivateExtension" }, function (response) {
            console.log(response);
            activateExtBtn.querySelector('path').setAttribute('stroke', '#a4a4a4');
            activated = false;  
        });
        else chrome.runtime.sendMessage({ action: "activateExtension" }, function (response) {
            console.log(response);
